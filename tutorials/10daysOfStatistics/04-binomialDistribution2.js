...
const main = () => {
  const an = readLine().split(' ');
  const a = parseInt(an[0], 10);
  const b = 100 - a;

  const pa = a / (a + b);
  const pb = b / (a + b);

  const n = parseInt(an[1], 10);

  const lowerLimitA = 0;
  const upperLimitA = 2;
  console.log(cumulativeProbability(lowerLimitA, upperLimitA, n, pa, pb).toFixed(3));

  const lowerLimitB = 2;
  const upperLimitB = 10;
  console.log(cumulativeProbability(lowerLimitB, upperLimitB, n, pa, pb).toFixed(3));
};
